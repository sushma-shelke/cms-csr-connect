...
export function MISReportManager() {
  ...
  // Calculate total targets and performance for filtered items using backend data
        ...
        monthlyTargets: item.monthlyTargets.map(target => {
          if (target.id === targetId) {
            ...
            console.log(`🎯 Updated Target: Item ${item.srNo}, Month ${target.targetMonth}, ` +
              ...
              `Deviation: ${target.deviation}, Achievement: ${target.achievementPercentage}%`);
            
            return {
              ...target,
              achievedTarget: achievedValue
              // Deviation and achievementPercentage are calculated by backend
            };
          }
          return target;
        })
    ...
    if (item.monthlyTargets) {
      item.monthlyTargets.forEach(target => {
        monthlyTargetItems.push({
          budgetAllocationItemSrNo: item.srNo,
          ...
          deviation: target.deviation, // From backend
          achievementPercentage: target.achievementPercentage // From backend
        });
      });
    }
...
// Submit report using the same update API
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Monthly reporting portal - {currentReportingPeriod.month} {currentReportingPeriod.year}
          </p>
        </div>
      </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 bg-muted/30 rounded-lg">
                                      ...
                                      <div>
                                        ...
                                        <div className={cn(
                                          ...
                                          currentMonthTarget.deviation < 0 ? "bg-impact-red/20 text-impact-red" : "bg-muted text-muted-foreground", "bg-muted"
                                        )}>
                                      ...
                                      {/* Achievement Percentage (From Backend) */}
                                      <div>
