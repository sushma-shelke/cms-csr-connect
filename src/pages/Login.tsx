...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById('recaptcha-container')) {
      ...
      recaptchaDiv.style.display = 'none'; // Hide the container
  ...
  // Lazy load Firebase OTP service
  useEffect(() => {
    if (loginMethod === 'otp' && !firebaseLoaded) {
      import('@/services/firebaseOTP').then(({ setupRecaptcha }) => {
        // Setup reCAPTCHA
        setupRecaptcha();
        setFirebaseLoaded(true);
        console.log('Firebase OTP service loaded and reCAPTCHA setup complete');
      }).catch(error => {
        console.error('Failed to load Firebase OTP service:', error);
        setUseFirebaseOTP(false);
        toast({
          title: "Firebase Warning",
          description: "Firebase OTP not available. Using backend OTP only.",
          variant: "destructive",
        });
      });
    }
  }, [loginMethod, firebaseLoaded]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      // Try client-side Firebase OTP generation first if available
      if (useFirebaseOTP && firebaseLoaded) {
        try {
          const { sendFirebaseOTP } = await import('@/services/firebaseOTP');
          const result = await sendFirebaseOTP(mobile);

          if (result.success) {
            setOtpSent(true);
            toast({
              title: "Firebase OTP Sent",
              description: `OTP has been sent to ${formattedPhoneNumber} via Firebase.`,
            });
            return;
          } else {
            console.log('Firebase OTP failed, falling back to backend:', result.message);
          }
        } catch (firebaseError) {
          console.error('Firebase OTP generation failed:', firebaseError);
          setUseFirebaseOTP(false);
          toast({
            title: "Firebase OTP Failed",
            description: "Falling back to backend OTP generation.",
            variant: "destructive",
          });
        }
      }
      ...
      if (response.ok) {
        ...
        toast({
          title: "OTP Sent",
          description: `OTP has been sent to ${formattedPhoneNumber} ${useFirebaseOTP ? 'via Firebase' : ''}.`,
        });
      } else {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // Try client-side Firebase verification first if available and OTP was sent via Firebase
      if (useFirebaseOTP && firebaseLoaded) {
        try {
          const { verifyFirebaseOTP } = await import('@/services/firebaseOTP');
          ...
          if (result.success) {
            ...
            try {
              const response = await fetch('https://mumbailocal.org:8089/api/auth/firebase-login', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
              ...
              if (response.ok) {
                ...
                // Create user session in auth context
                const success = await loginWithOtp(formattedPhoneNumber);

                if (success) {
                  toast({
                    title: "Firebase Login Successful",
                    description: "You have successfully logged in with Firebase OTP.",
                  });
                ...
                toast({
                  ...
                  description: data.message || "Backend authentication failed. Please try again.",
                  variant: "destructive",
                });
              }
            } catch (backendError) {
              ...
              toast({
                title: "Backend Error",
                description: "Failed to authenticate with backend. Please try again.",
                variant: "destructive",
              });
            }
            ...
            console.log('Firebase OTP verification failed, falling back to backend:', result.message);
          }
        } catch (firebaseError) {
          console.error('Firebase OTP verification failed:', firebaseError);
          setUseFirebaseOTP(false);
          toast({
            title: "Firebase Verification Failed",
            description: "Falling back to backend OTP verification.",
            variant: "destructive",
          });
        }
      }
      ...
      if (validateResponse.ok) {
        // Login with OTP in auth context
        const success = await loginWithOtp(formattedPhoneNumber);

        if (success) {
          toast({
            title: "Login Successful",
            description: `You have successfully logged in ${useFirebaseOTP ? 'with Firebase-enhanced' : ''} OTP.`,
          });
